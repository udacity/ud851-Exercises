...
public class MainActivity extends AppCompatActivity implements
    ...
    protected void onResume() {
        ...
        // COMPLETED Check if you are on Android M or later, if so...
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            // COMPLETED Get a BatteryManager instance using getSystemService()
            // COMPLETED Call isCharging on the battery manager and pass the result on to your show
            // charging method
            BatteryManager batteryManager = (BatteryManager) getSystemService(BATTERY_SERVICE);
            showCharging(batteryManager.isCharging());
        }
        else {
            // COMPLETED If your user is not on M+, then...
            // COMPLETED Create a new intent filter with the action ACTION_BATTERY_CHANGED. This is a
            // sticky broadcast that contains a lot of information about the battery state.
            // COMPLETED Set a new Intent object equal to what is returned by registerReceiver, passing in null
            ...
            // COMPLETED Get the integer extra BatteryManager.EXTRA_STATUS. Check if it matches
            // BatteryManager.BATTERY_STATUS_CHARGING or BatteryManager.BATTERY_STATUS_FULL. This means
            // the battery is currently charging.
            // COMPLETED Update the UI using your showCharging method
            IntentFilter intentFilter = new IntentFilter(Intent.ACTION_BATTERY_CHANGED);
            Intent intent = registerReceiver(null, intentFilter);
            int status = intent.getIntExtra(BatteryManager.EXTRA_STATUS, 0);
            boolean isCharging = (BatteryManager.BATTERY_STATUS_CHARGING == status) || (BatteryManager.BATTERY_STATUS_FULL == status);
